...
before(async () => {
  const frontendUrl = await listen(http.createServer((request, response) => {
    if (request.url === '/health') {
      jsonResponse(response, { status: 'UP', service: 'frontend', version: 'web-test', imageTag: 'frontend-image-test' });
      return;
    }
  ...
  const userUrl = await listen(http.createServer((request, response) => {
    if (request.url === '/health') {
      jsonResponse(response, { status: 'UP', service: 'user-service', version: 'user-test', imageTag: 'user-image-test' });
      return;
    }
  ...
  const catalogUrl = await listen(http.createServer((request, response) => {
    if (request.url === '/health') {
      jsonResponse(response, { status: 'UP', service: 'catalog-service', version: 'catalog-test', imageTag: 'catalog-image-test' });
      return;
    }
  ...
  const orderUrl = await listen(http.createServer((request, response) => {
    if (request.url === '/health') {
      jsonResponse(response, { status: 'UP', service: 'order-service', version: 'order-test', imageTag: 'order-image-test' });
      return;
    }
    const chunks = [];
...
test('reports consolidated service versions and image tags', async () => {
  ...
  assert.equal(body.imageTag, 'gateway-image-test');
  assert.match(body.generatedAt, /^\d{4}-\d{2}-\d{2}T/);
  assert.deepEqual(body.services, [
    { service: 'frontend', status: 'UP', version: 'web-test', imageTag: 'frontend-image-test' },
    { service: 'user-service', status: 'UP', version: 'user-test', imageTag: 'user-image-test' },
    { service: 'catalog-service', status: 'UP', version: 'catalog-test', imageTag: 'catalog-image-test' },
    { service: 'order-service', status: 'UP', version: 'order-test', imageTag: 'order-image-test' }
  ]);
});
